...
<head>
    ...
    @endphp
    <title>{{get_customer($recorde->customer_id)->customer_name}} | Return Product Invoice</title>
    <!-- Required meta tags -->
    <meta charset="utf-8">
...
<body>
        ...
        <div class="page-inner">
                            ...
                            <tbody>
                                ...
                                @foreach ($recorde->data as $key=>$value)
                                    <tr class="border">
                                        ...
                                        <td>{{ $value['Sale_product_quantity'] }}</td>
                                        <td>{{ $value['receivedQuantity'] }}</td>
                                        <td>{{ number_format($value['product_Sale_price']) }}</td>
                                        @php
                                            $q+=$value['receivedQuantity'];
                                            $t+= $value['Sale_product_quantity']*$value['product_Sale_price'];
                                            $index=$key;
                                            $total1=$t;
                                        @endphp
                                        <td>{{ number_format($value['product_Sale_price']*$value['receivedQuantity'])}}</td>
                                    </tr>
                                @endforeach
                            </tbody>
                        </table>
            ...
            <div class="card-body">
                <div class="row">
                    ...
                    <p class="col-12 text-left"><b>Total Return Amount: </b>{{__(number_format($total))}}</p>
                    {{-- <p class="col-12 text-left"><b>Total Amount: </b>{{__(number_format($stock[$index]['totalamount']))}}</p> --}}
                    {{-- <p class="col-12 text-left"><b>Total Return Amount:: </b>&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;{{__(number_format($total))}}</p> --}}
                    {{-- <p class="col-12 text-left" style="margin-bottom: 10vh"><b>Remaining: </b>&nbsp;&nbsp;&nbsp;{{__(number_format($total-$total1))}}</p> --}}
                </div>
            </div>
        </div>
    ...
    <script>
        $(document).ready(function() {
            window.print();
            setTimeout(function() {
                // window.location.href = "{{ route('admin.returned_product.index') }}"
            }, 1000);
        });
    </script>
</body>
